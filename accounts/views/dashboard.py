...
def ceo_dashboard(request):
    ...
    # Employees
    total_employees = User.objects.filter(organization=organization, is_active=True).count()
    active_employees = User.objects.filter(
        organization=organization,
        is_active=True
    ).exclude(role__name='CEO').count()
    ...
    # Leads summary
    leads_summary = {}
    for stage in ['NEW', 'CONTACTED', 'PROPOSAL_SENT', 'NEGOTIATION', 'CLOSED_WON', 'CLOSED_LOST']:
        leads = Lead.objects.filter(organization=organization, stage=stage)
        leads_summary[stage] = {
            'count': leads.count(),
            'revenue': leads.aggregate(Sum('expected_revenue'))['expected_revenue__sum'] or 0
        }
...
def hr_dashboard(request):
    ...
    employees_by_role = User.objects.filter(
        organization=organization,
        is_active=True
    ).exclude(role__isnull=True).values('role__name').annotate(
        count=Count('id')
    )
    employees_by_role_dict = {item['role__name']: item['count'] for item in employees_by_role}
    ...
    # Leave types chart
    leave_types = Leave.objects.filter(organization=organization).values('leave_type').annotate(
        count=Count('id')
    )
...
def pm_dashboard(request):
    ...
    team_activity = DailyUpdate.objects.filter(
        project_id__in=my_projects_ids
    ).order_by('-created_at')[:5]
...
def bde_dashboard(request):
    ...
    # Expected revenue
    expected_revenue = leads.aggregate(
