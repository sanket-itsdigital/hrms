...
def create_user(request):
    ...
    if request.method == "POST":
        ...
        form = UserForm(
            request.POST, request.FILES, user=user, organization=user.organization
        )
        if form.is_valid():
            ...
            new_user.save()
            messages.success(
                request, f"User {new_user.get_full_name()} created successfully!"
            )
            return redirect("accounts:users")
    else:
        from accounts.forms import UserForm
    ...
    return render(
        request, "accounts/users/form.html", {"form": form, "title": "Create New User"}
    )
...
def update_user(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            updated_user.save()
            messages.success(
                request, f"User {updated_user.get_full_name()} updated successfully!"
            )
            return redirect("accounts:users")
    else:
        from accounts.forms import UserForm
    ...
    return render(
        ...
        {"form": form, "target_user": target_user, "title": "Update User"},
    )
...
def delete_user(request, id):
    ...
    return render(
        request, "accounts/users/delete_confirm.html", {"target_user": target_user}
    )
