...
def list_attendance(request):
    ...
    if user.is_ceo or user.is_hr:
        ...
        ).select_related('user', 'marked_by').order_by('-date', 'user__first_name')
    else:
        # Others see only their own attendance
        attendances = Attendance.objects.filter(
            user=user,
            organization=organization
        ).select_related('user', 'marked_by').order_by('-date')
...
def create_attendance(request):
    ...
    if not (user.is_ceo or user.is_hr):
        messages.error(request, 'You do not have permission to create attendance records.')
        return redirect('attendance:my-attendance')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            attendance.save()
            messages.success(request, f'Attendance record created successfully for {attendance.user.get_full_name()}!')
...
def update_attendance(request, id):
    ...
    if request.method == 'POST':
        form = AttendanceForm(request.POST, instance=attendance, user=user, organization=user.organization)
        if form.is_valid():
            attendance = form.save(commit=False)
            if user.is_ceo or user.is_hr:
                attendance.marked_by = user
        ...
        form = AttendanceForm(instance=attendance, user=user, organization=user.organization)
...
def delete_attendance(request, id):
    ...
    if request.method == 'POST':
        ...
        attendance.delete()
        messages.success(request, f'Attendance record for {user_name} deleted successfully!')
        return redirect('attendance:list')
...
def bulk_attendance(request):
    ...
    if not (user.is_ceo or user.is_hr):
        messages.error(request, 'You do not have permission to perform bulk attendance entry.')
        return redirect('attendance:my-attendance')
    ...
    if request.method == 'POST':
        form = AttendanceBulkForm(request.POST, user=user, organization=user.organization)
        if form.is_valid():
    ...
    return render(request, 'attendance/bulk_form.html', {
        'form': form,
...
def export_attendance(request):
    ...
    writer.writerow([
        'Date', 'Employee Name', 'Email', 'Status', 'Check In', 'Check Out', 
        'Notes', 'Marked By', 'Created At'
    ])
