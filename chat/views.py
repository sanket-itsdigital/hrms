...
def chat_room(request, room_id):
    ...
    messages_list = Message.objects.filter(room=room).select_related('sender').order_by('created_at')
...
def create_project_chat(request, project_id):
    ...
    if not has_access:
        messages.error(request, 'You do not have access to create a chat for this project.')
        return redirect('chat:list')
    ...
    if created:
        ...
        messages.info(request, f'Chat room already exists for project "{project.name}"!')
...
def create_personal_chat(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Started chat with {other_user.get_full_name() or other_user.username}!')
            return redirect('chat:room', room_id=chat_room.id)
    else:
