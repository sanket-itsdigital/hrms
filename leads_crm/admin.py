...
class LeadAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return qs.select_related(
            "assigned_to", "created_by", "organization", "converted_to_project"
    ...
    def assigned_users_display(self, obj):
        ...
        if not users:
            return "-"
        return ", ".join([user.get_full_name() or user.email for user in users])
...
class ProposalAdmin(admin.ModelAdmin):
    ...
    def preview_file(self, obj):
        if obj.proposal_file:
            return format_html(
                '<a href="{}" target="_blank">View Proposal</a>', obj.proposal_file.url
            )
        return "No file"
...
class MeetingLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Meeting Details", {"fields": ("notes", "logged_by")}),
